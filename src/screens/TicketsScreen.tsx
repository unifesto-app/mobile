...
export default function TicketsScreen() {
  ...
  const loadTickets = async (pageNum: number = 1, reset: boolean = false) => {
    try {
      ...
      const allEvents = (response?.data || response?.events || []).map((r: any) => r.event ? { ...r.event, qrCode: r.qrCode, registrationId: r.id, registrationStatus: r.status, paymentStatus: r.paymentStatus, ticketType: r.ticketType, ticketCode: r.tickets?.[0]?.ticketCode } : r);
